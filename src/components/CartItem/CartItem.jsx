...
export const CartItem = ({ id, img, name, price, cartCount }) => {

  const [width, setWidth] = useState(window.innerWidth);
  const breackpoint = 690;
  const { removeFromBasket} = useContext(ShopContext);
  useEffect(() => {
    ...
    return (
      <div className={style.cart}>
        ...
        <div className={style.info}>
          ...
          <button className={style.close} onClick={() => removeFromBasket(id)}>
            <Icon20Cancel />
            </button>
        </div>
      </div>
  ...
  return (
    <div className={style.cart}>
      ...
      <div className={style.info}>
        ...
        <button className={style.buttonBlue} onClick={() => removeFromBasket(id)}>
